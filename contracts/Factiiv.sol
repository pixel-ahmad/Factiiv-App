...
import "./@opengsn/_BaseRelayRecipient.sol";
...
contract Factiiv is FactiivStore, BaseRelayRecipient  , AccessControl{
  ...
  function updateAttestion(address subject, bytes32 id, string memory payload) external onlyRole(ATTESTOR_ROLE) {
  ...
  function arbitrationDelete(bytes32 id) external onlyRole(ARBITRATOR_ROLE) {
    _deleteRelationship(_msgSender(), id);
  }
